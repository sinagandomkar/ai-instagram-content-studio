...
- `npm run dev` + `curl`: `/api/dashboard`, `/api/settings`, `/api/library` return `200` against a freshly migrated Postgres DB (originally verified against SQLite before the Postgres switch; same result).
